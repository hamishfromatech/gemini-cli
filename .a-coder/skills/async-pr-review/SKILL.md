...
3.  **Ephemeral Worktrees**: Instead of checking out branches in the user's main workspace, the skill provisions temporary git worktrees in `.a-coder/tmp/async-reviews/pr-<number>`. This prevents git lock conflicts and namespace pollution.
...
2.  Execute the `async-review.sh` script, passing the PR number as the first argument. Be sure to run it with the `is_background` flag set to true to ensure it immediately detaches.
    ```bash
    .a-coder/skills/async-pr-review/scripts/async-review.sh <PR_NUMBER>
    ```
...
2.  Execute the `check-async-review.sh` script, passing the PR number as the first argument:
    ```bash
    .a-coder/skills/async-pr-review/scripts/check-async-review.sh <PR_NUMBER>
    ```
