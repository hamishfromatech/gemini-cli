...
## 🚨 CRITICAL: macOS Daemon Workaround & A-Coder CLI Usage 🚨
...
### Testing the A-Coder CLI

When testing the A-Coder CLI with `agent-tui`, there are several strict requirements to ensure deterministic and accurate behavior:
...
2. **Bypass Trust Modals**: Always pass `A_CODER_CLI_TRUST_WORKSPACE=true` in the environment. If you don't, any new project-level agents or extensions will trigger a full-screen "Acknowledge and Enable" modal. This modal steals focus, swallows automation keystrokes, and causes `agent-tui wait` commands to time out.
3. **Isolated Environments**: If you need to test without real user credentials or existing agents interfering, isolate the global settings using `A_CODER_CLI_HOME=<some-test-dir>`.
...
env A_CODER_CLI_TRUST_WORKSPACE=true A_CODER_CLI_HOME=test-gemini-home agent-tui run -d "$(pwd)" node packages/cli/dist/index.js
```
