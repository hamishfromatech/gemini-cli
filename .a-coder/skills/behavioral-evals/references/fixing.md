...
1.  **Fetch Nightly Results**: Use the `gh` CLI to inspect the latest run from
    ...
    - _Example view URL_:
      `https://github.com/hamishfromatech/a-coder-cli/actions/workflows/evals-nightly.yml`
...
3.  **Read Logs**:
    ...
    - Enable verbose debugging via `export A_CODER_DEBUG_LOG_FILE="debug.log"`.
...
3.  **Assertion Fidelity**:
    ...
    - Test fixes should not "cheat" by changing a test's `A_CODER.md` file or by
