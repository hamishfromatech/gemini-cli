...
```bash
export A_CODER_API_KEY=$(grep '^A_CODER_API_KEY=' .env | cut -d '=' -f2) && RUN_EVALS=1 npx vitest run --config evals/vitest.config.ts <file_name>
```
...
- **Verbose Reasoning**: Capture raw buffer traces by setting
  `A_CODER_DEBUG_LOG_FILE`:
  ```bash
  export A_CODER_DEBUG_LOG_FILE="debug.log"
  ```
