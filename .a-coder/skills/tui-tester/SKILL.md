---
name: tui-tester
description: Expert guidance for testing A-Coder CLI behavior and visual output using terminal automation.
---

# TUI Tester Skill

This skill provides the operational manual for verifying A-Coder CLI behavioral changes and visual output using terminal automation.
...
### 4. A-Coder CLI Specifics
- **Build First**: Always run `npm run build` or `npm run build:all` before testing local changes.
- **Bypass Trust**: Set `A_CODER_CLI_TRUST_WORKSPACE=true` to avoid focus-stealing modals.
- **Isolate Config**: Use `A_CODER_CLI_HOME` to prevent interference with your personal settings.

## Workflow Example

```bash
# Start the CLI
env A_CODER_CLI_TRUST_WORKSPACE=true agent-tui run node packages/cli/dist/index.js
