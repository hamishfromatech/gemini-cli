...
export const DEFAULT_EXCLUDED_ENV_VARS = [
  'DEBUG',
  'DEBUG_MODE',
  'A_CODER_CLI_IDE_SERVER_STDIO_COMMAND',
  'A_CODER_CLI_IDE_SERVER_STDIO_ARGS',
];
...
export function getSystemDefaultsPath(): string {
  if (process.env['A_CODER_CLI_SYSTEM_DEFAULTS_PATH']) {
    return process.env['A_CODER_CLI_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
...
function findEnvFile(
  ...
  while (true) {
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      if (isTrusted) {
        const homeGeminiEnvPath = path.join(homedir(), A_CODER_DIR, '.env');
...
const USER_GCP_PROJECT = '_A_CODER_USER_GCP_PROJECT';
...
export function loadEnvironment(
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(A_CODER_DIR);
