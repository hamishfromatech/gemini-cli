...
to the A-Coder CLI Policy Engine.
...
- `a-coder-extension.json`: The manifest file.
- `policies/`: Contains the `.toml` policy files.

## How to use

1.  Link this extension to your local A-Coder CLI installation:

    ```bash
    a-coder-cli extensions link packages/cli/src/commands/extensions/examples/policies
    ```

2.  Restart your A-Coder CLI session.
...
For security, A-Coder CLI ignores any `allow` decisions or `yolo` mode
