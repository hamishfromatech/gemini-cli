...
import { debugLogger, writeToStdout } from '@the-a-tech-corporation/core';
...
function sanitizeNotificationContent(
  ...
  return {
    title: title || 'A-Coder CLI',
    subtitle: subtitle || undefined,
    body: body || 'Open A-Coder CLI for details.',
  };
}
...
export function buildRunEventNotificationContent(
  ...
  if (event.type === 'attention') {
    return sanitizeNotificationContent({
      title: 'A-Coder CLI needs your attention',
      subtitle: event.heading ?? 'Action required',
      body: event.detail ?? 'Open A-Coder CLI to continue.',
    });
  }

  return sanitizeNotificationContent({
    title: 'A-Coder CLI session complete',
