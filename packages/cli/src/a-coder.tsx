...
export async function main() {
  ...
  if (
    ...
    coreEvents.emitFeedback(
      'warning',
      'Warning: --allowed-tools cli argument and tools.allowed in settings.json are deprecated and will be removed in 1.0: Migrate to Policy Engine: https://a-coder-cli.com/docs/core/policy-engine/',
    );
  }
  ...
  if (
    ...
    coreEvents.emitFeedback(
      'warning',
      'Warning: tools.exclude in settings.json is deprecated and will be removed in 1.0. Migrate to Policy Engine: https://a-coder-cli.com/docs/core/policy-engine/',
    );
  }
  ...
  if (!settings.merged.security.auth.useExternal && !argv.isCommand) {
    try {
      ...
      const providerBaseUrl = settings.merged.provider?.baseUrl;
  ...
  // to run A-Coder CLI. It is now safe to perform expensive initialization that
