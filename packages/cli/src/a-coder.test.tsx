...
describe('a-coder.tsx main function', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('A_CODER_SANDBOX', '');
    vi.stubEnv('SANDBOX', '');
    vi.stubEnv('SHPOOL_SESSION_NAME', '');
    vi.stubEnv('A_CODER_CLI_TRUST_WORKSPACE', 'true');
...
describe('initializeOutputListenersAndFlush', () => {
  ...
  it('should flush backlogs and setup listeners if no listeners exist', async () => {
    const { coreEvents } = await import('@the-a-tech-corporation/core');
    const { initializeOutputListenersAndFlush } = await import('./a-coder.js');
...
describe('getNodeMemoryArgs', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env['A_CODER_CLI_NO_RELAUNCH'];
  ...
  it('should return empty array if A_CODER_CLI_NO_RELAUNCH is set', () => {
    process.env['A_CODER_CLI_NO_RELAUNCH'] = 'true';
...
describe('a-coder.tsx main function kitty protocol', () => {
  ...
  beforeEach(() => {
    ...
    originalEnvNoRelaunch = process.env['A_CODER_CLI_NO_RELAUNCH'];
    process.env['A_CODER_CLI_NO_RELAUNCH'] = 'true';
  ...
  afterEach(() => {
    ...
    if (originalEnvNoRelaunch !== undefined) {
      process.env['A_CODER_CLI_NO_RELAUNCH'] = originalEnvNoRelaunch;
    } else {
      delete process.env['A_CODER_CLI_NO_RELAUNCH'];
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
describe('resolveSessionId', () => {
  ...
  it('should import from session file when sessionFile is provided', async () => {
    ...
    const coreModule = await import('@the-a-tech-corporation/core');
...
describe('a-coder.tsx main function exit codes', () => {
  let originalEnvNoRelaunch: string | undefined;
  let originalIsTTY: boolean | undefined;
...
describe('startInteractiveUI', () => {
  ...
  it('should enable mouse events when alternate buffer is enabled', async () => {
    const { enableMouseEvents } = await import('@the-a-tech-corporation/core');
    await startTestInteractiveUI(
      mockConfig,
      mockSettings,
      mockStartupWarnings,
  ...
  it('should perform all startup tasks in correct order', async () => {
    const { getVersion } = await import('@the-a-tech-corporation/core');
  ...
  it('should not recordSlowRender when less than threshold', async () => {
    const { recordSlowRender } = await import('@the-a-tech-corporation/core');
