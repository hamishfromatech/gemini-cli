...
const SKILL_DESTINATION_CHOICES: DestinationChoice[] = [
  {
    destination: 'global',
    label: 'Global',
    description: '~/.a-coder/skills — available in all projects',
  },
  {
    destination: 'project',
    label: 'Project',
    description: '.a-coder/skills — available in this workspace',
  },
];
...
// project (private MEMORY.md and sibling files, plus ~/.a-coder/A-Coder.md),
...
function getSkillOriginTag(filePath: string): string {
  ...
  if (normalizedPath.includes('/.a-coder/skills/')) {
    ...
    if (
      homeDirs.some((homeDir) =>
        normalizedPath.startsWith(`${homeDir}/.a-coder/skills/`),
      )
    ) {
      return 'Global';
    }
