...
import { ACoderRespondingSpinner } from './ACoderRespondingSpinner.js';
...
vi.mock('./ACoderSpinner.js', () => ({
  ACoderSpinner: ({ altText }: { altText?: string }) => (
    <Text>ACoderSpinner {altText}</Text>
  ),
}));

describe('ACoderRespondingSpinner', () => {
  ...
  it('renders spinner when responding', async () => {
    ...
    const { lastFrame, unmount } = await render(<ACoderRespondingSpinner />);
    expect(lastFrame()).toContain('ACoderSpinner');
    unmount();
  });
  ...
  it('renders non-responding display when provided', async () => {
    ...
    const { lastFrame, unmount } = await render(
      <ACoderRespondingSpinner nonRespondingDisplay="Waiting..." />,
    );
