...
import { useACoderStream } from './useACoderStream.js';
...
const MockedACoderClientClass = vi.hoisted(() =>
...
vi.mock('@the-a-tech-corporation/core', async (importOriginal) => {
  ...
  return {
    ...
    GitService: vi.fn(),
    ACoderClient: MockedACoderClientClass,
...
// --- Tests for useACoderStream Hook ---
describe('useACoderStream', () => {
  ...
  const mockGetACoderClient = vi.fn().mockImplementation(() => {
    const clientInstance = new MockedACoderClientClass(mockConfig);
  ...
  beforeEach(() => {
    ...
    // Reset mocks for ACoderClient instance methods (startChat and sendMessageStream)
    // The ACoderClient constructor itself is mocked at the module level.
    mockStartChat.mockClear().mockResolvedValue({
      sendMessageStream: mockSendMessageStream,
    } as unknown as any); // ACoderChat -> any
  ...
  const mockLoadedSettings: LoadedSettings = {
    ...
    workspace: { path: '/workspace/.a-coder/settings.json', settings: {} },
    errors: [],
    forScope: vi.fn(),
    setValue: vi.fn(),
  ...
  const renderTestHook = async (
    ...
    const client = aCoderClient || mockConfig.getACoderClient();
  ...
  const renderHookWithDefaults = async (
    ...
    return renderHookWithProviders(() =>
      useACoderStream(
        new MockedACoderClientClass(mockConfig),
        [],
        mockAddItem,
        mockConfig,
        mockLoadedSettings,
  ...
  it('should inject steering hint prompt for continuation', async () => {
    ...
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerACoderEventType.Content,
  ...
  it('should handle all tool calls being cancelled', async () => {
    ...
    const client = new MockedACoderClientClass(mockConfig);
  ...
  describe('Retry Handling', () => {
    ...
    it('should reset retryStatus when isResponding becomes false', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: ServerACoderEventType.Content, value: 'Part 1' };
        await new Promise(() => {}); // Keep stream open
      })();
      mockSendMessageStream.mockReturnValue(mockStream);
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should record client-initiated tool calls in ACoderChat history', async () => {
      const { result, client: mockACoderClient } = await renderTestHook();
      ...
      // Verify that the tool call and response were added to ACoderChat history
      expect(mockACoderClient.addHistory).toHaveBeenCalledWith({
        role: 'model',
        parts: [
          {
            functionCall: {
    ...
    it('should NOT record other client-initiated tool calls in history', async () => {
      ...
      expect(mockACoderClient.addHistory).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Error Handling', () => {
    it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
      ...
      const { result } = await renderHookWithProviders(() =>
        useACoderStream(
          new MockedACoderClientClass(testConfig),
          [],
          mockAddItem,
          testConfig,
          mockLoadedSettings,
  ...
  describe('handleFinishedEvent', () => {
    it('should add info message for MAX_TOKENS finish reason', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerACoderEventType.Finished,
      ...
      it.each([
        ...
        async ({ requestTokens, remainingTokens, expectedMessage }) => {
          mockSendMessageStream.mockReturnValue(
            (async function* () {
              yield {
                type: ServerACoderEventType.ContextWindowWillOverflow,
                value: {
    ...
    it('should add informational messages when ChatCompressed event is received', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerACoderEventType.ChatCompressed,
            value: {
  ...
  it('should flush pending text rationale before scheduling tool calls to ensure correct history order', async () => {
    ...
    const mockStream = (async function* () {
      ...
      yield {
        type: ServerACoderEventType.ToolCallRequest,
  ...
  describe('Thought Reset', () => {
    it('should keep full thinking entries in history when mode is full', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerACoderEventType.Thought,
            value: {
    ...
    it('should reset thought to null when user cancels', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield { type: ServerACoderEventType.UserCancelled };
        })(),
      );
    ...
    it('should reset thought to null when there is an error', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerACoderEventType.Error,
  ...
  describe('Loop Detection Confirmation', () => {
    ...
    it('should set loopDetectionConfirmationRequest when LoopDetected event is received', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerACoderEventType.LoopDetected,
          };
        })(),
      );
    ...
    it('should disable loop detection and show message when user selects "disable"', async () => {
      ...
      mockConfig.getACoderClient = vi.fn().mockReturnValue(mockClient);
  ...
  describe('Agent Execution Events', () => {
    it('should handle AgentExecutionStopped event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerACoderEventType.AgentExecutionStopped,
            value: {
              reason: 'hook-reason',
    ...
    it('should handle AgentExecutionBlocked event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerACoderEventType.AgentExecutionBlocked,
            value: {
              reason: 'hook-reason',
  ...
  describe('Stream Splitting', () => {
    it('should not add empty history item when splitting message results in empty or whitespace-only beforeText', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerACoderEventType.Content, value: 'test content' };
        })(),
      );
    ...
    it('should add whitespace-only history item when splitting message', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerACoderEventType.Content, value: '   content' };
        })(),
      );
  ...
  it('should trace UserPrompt telemetry on submitQuery', async () => {
    ...
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield { type: ServerACoderEventType.Content, value: 'Response' };
      })(),
    );
