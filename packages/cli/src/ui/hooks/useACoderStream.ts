...
import {
  ACoderEventType as ServerACoderEventType,
  getErrorMessage,
  isNodeError,
  MessageSenderType,
  logUserPrompt,
...
import type {
  ...
  ServerACoderContentEvent as ContentEvent,
  ServerACoderFinishedEvent,
  ServerACoderStreamEvent as GeminiEvent,
  ThoughtSummary,
  ToolCallRequestInfo,
  ToolCallResponseInfo,
  ACoderErrorEventValue,
  RetryAttemptPayload,
} from '@the-a-tech-corporation/core';
...
export const useACoderStream = (
  ...
  const [
    ...
    async (completedToolCallsFromScheduler) => {
      ...
      if (completedToolCallsFromScheduler.length > 0) {
        ...
        try {
          const currentModel =
            config.getACoderClient().getCurrentSequenceModel() ??
            config.getModel();
          config
            .getACoderClient()
            .getChat()
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < toolCalls.length; i++) {
      ...
      if (
        ...
        // Architecturally, `useACoderStream` should not be responsible for stitching
  ...
  const handleContentEvent = useCallback(
    (
      ...
      currentACoderMessageBuffer: string,
      userMessageTimestamp: number,
    ): string => {
      setRetryStatus(null);
      if (turnCancelledRef.current) {
      ...
      let newACoderMessageBuffer = currentACoderMessageBuffer + eventValue;
      if (
        ...
        newACoderMessageBuffer = eventValue;
      }
      // Split large messages for better rendering performance. Ideally,
      // we should maximize the amount of output sent to <Static />.
      const splitPoint = findLastSafeSplitPoint(newACoderMessageBuffer);
      if (splitPoint === newACoderMessageBuffer.length) {
        // Update the existing message with accumulated content
        setPendingHistoryItem((item) => ({
          // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
          type: item?.type as 'gemini' | 'gemini_content',
          text: newACoderMessageBuffer,
        }));
      } else {
        ...
        const beforeText = newACoderMessageBuffer.substring(0, splitPoint);
        const afterText = newACoderMessageBuffer.substring(splitPoint);
        ...
        newACoderMessageBuffer = afterText;
      }
      return newACoderMessageBuffer;
    },
  ...
  const handleErrorEvent = useCallback(
    (eventValue: ACoderErrorEventValue, userMessageTimestamp: number) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
        setPendingHistoryItem(null);
      }
  ...
  const handleFinishedEvent = useCallback(
    (event: ServerACoderFinishedEvent, userMessageTimestamp: number) => {
  ...
  const handleChatCompressionEvent = useCallback(
    (
      eventValue: ServerACoderChatCompressedEvent['value'],
      userMessageTimestamp: number,
    ) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
  ...
  const processACoderStreamEvents = useCallback(
    async (
      ...
      for await (const event of stream) {
        if (
          event.type !== ServerACoderEventType.Thought &&
        ...
        switch (event.type) {
          case ServerACoderEventType.Thought:
            setLastGeminiActivityTime(Date.now());
            handleThoughtEvent(event.value, userMessageTimestamp);
            break;
          case ServerACoderEventType.Content:
            setLastGeminiActivityTime(Date.now());
            ...
            break;
          case ServerACoderEventType.ToolCallRequest:
            toolCallRequests.push(event.value);
            break;
          case ServerACoderEventType.UserCancelled:
            handleUserCancelledEvent(userMessageTimestamp);
            break;
          case ServerACoderEventType.Error:
            handleErrorEvent(event.value, userMessageTimestamp);
            break;
          case ServerACoderEventType.AgentExecutionStopped:
            ...
            break;
          case ServerACoderEventType.AgentExecutionBlocked:
            ...
            break;
          case ServerACoderEventType.ChatCompressed:
            handleChatCompressionEvent(event.value, userMessageTimestamp);
            break;
          case ServerACoderEventType.ToolCallConfirmation:
          case ServerACoderEventType.ToolCallResponse:
            // do nothing
            break;
          case ServerACoderEventType.MaxSessionTurns:
            handleMaxSessionTurnsEvent();
            break;
          case ServerACoderEventType.ContextWindowWillOverflow:
            handleContextWindowWillOverflowEvent(
              event.value.estimatedRequestTokenCount,
              event.value.remainingTokenCount,
            );
            break;
          case ServerACoderEventType.Finished:
            handleFinishedEvent(event, userMessageTimestamp);
            break;
          case ServerACoderEventType.Citation:
            handleCitationEvent(event.value, userMessageTimestamp);
            break;
          case ServerACoderEventType.ModelInfo:
            handleChatModelEvent(event.value, userMessageTimestamp);
            break;
          case ServerACoderEventType.LoopDetected:
            // handle later because we want to move pending history to history
            // before we add loop detected message to history
            loopDetectedRef.current = true;
            break;
          case ServerACoderEventType.Retry:
          case ServerACoderEventType.InvalidStream:
      ...
      runInDevTraceSpan(
        ...
        async ({ metadata: spanMetadata }) => {
          ...
          return promptIdContext.run(prompt_id, async () => {
            ...
            try {
              const stream = aCoderClient.sendMessageStream(
                queryToSend,
                abortSignal,
                prompt_id!,
                undefined,
              ...
              const processingStatus = await processACoderStreamEvents(
                stream,
                userMessageTimestamp,
                abortSignal,
              );
  ...
  const handleApprovalModeChange = useCallback(
    ...
    [config, toolCalls, aCoderClient, streamingState, addItem, onDebugMessage],
  );
  ...
  }, [toolCalls, config, onDebugMessage, gitService, history, aCoderClient]);
