...
for the A-Coder CLI. The ACP allows external clients (like IDE extensions) to
communicate with the A-Coder CLI agent over a structured JSON-RPC based protocol.
...
```bash
# General pattern
npm test -w @the-a-tech-corporation/a-coder-cli -- src/acp/<test-file-name>.ts

# Example
npm test -w @the-a-tech-corporation/a-coder-cli -- src/acp/acpRpcDispatcher.test.ts
```
...
```bash
source ~/.nvm/nvm.sh && nvm use default && npm test -w @the-a-tech-corporation/a-coder-cli -- src/acp/acpSession.test.ts
```
