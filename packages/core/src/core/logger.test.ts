...
const TEST_A_CODER_DIR = path.join(
  os.homedir(),
  A_CODER_DIR,
  TMP_DIR_NAME,
  PROJECT_SLUG,
...
const TEST_LOG_FILE_PATH = path.join(TEST_A_CODER_DIR, LOG_FILE_NAME);
...
async function cleanupLogAndCheckpointFiles() {
  try {
    await fs.rm(TEST_A_CODER_DIR, { recursive: true, force: true });
  } catch {
...
describe('Logger', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(TEST_A_CODER_DIR, { recursive: true });
  ...
  describe('initialize', () => {
    it('should create .a-coder directory and an empty log file if none exist', async () => {
      const dirExists = await fs
        .access(TEST_A_CODER_DIR)
        .then(() => true)
        .catch(() => false);
    ...
    it('should handle invalid JSON in log file by backing it up and starting fresh', async () => {
      ...
      const logContent = await readLogFile();
      expect(logContent).toEqual([]);
      const dirContents = await fs.readdir(TEST_A_CODER_DIR);
      expect(
        dirContents.some(
          (f) =>
