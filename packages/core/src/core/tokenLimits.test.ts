...
describe('tokenLimit', () => {
  it('should return the correct token limit for default models', () => {
    expect(tokenLimit(DEFAULT_A_CODER_MODEL)).toBe(1_048_576);
    expect(tokenLimit(DEFAULT_A_CODER_FLASH_MODEL)).toBe(1_048_576);
    expect(tokenLimit(DEFAULT_A_CODER_FLASH_LITE_MODEL)).toBe(1_048_576);
  });

  it('should return the correct token limit for preview models', () => {
    expect(tokenLimit(PREVIEW_A_CODER_MODEL)).toBe(1_048_576);
    expect(tokenLimit(PREVIEW_A_CODER_FLASH_MODEL)).toBe(1_048_576);
  });
