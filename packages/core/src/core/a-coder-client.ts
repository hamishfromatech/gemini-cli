...
export class ACoderClient {
  private chat?: ACoderChat;
  ...
  private _getActiveModelForCurrentTurn(): string {
    ...
    return resolveModel(
      ...
      this.config.getACoder31LaunchedSync?.() ?? false,
      false,
      this.config.getHasAccessToPreviewModel?.() ?? true,
      this.config,
      this.config.hasACoder35FlashGAAccess?.() ?? false,
    );
  }
  ...
  private async *processTurn(
  ...
  ): AsyncGenerator<ServerACoderStreamEvent, Turn> {
    ...
    if (
      ...
      yield { type: ACoderEventType.MaxSessionTurns };
      return turn;
    }
    ...
    if (this.config.getContextManagementConfig().enabled) {
      ...
      if (compressed.compressionStatus === CompressionStatus.COMPRESSED) {
        yield { type: ACoderEventType.ChatCompressed, value: compressed };
      }
    }
    ...
    if (loopResult.count > 1) {
      yield { type: ACoderEventType.LoopDetected };
      return turn;
    ...
    if (!signal.aborted && !this.currentSequenceModel) {
      yield { type: ACoderEventType.ModelInfo, value: modelToUse };
    }
    ...
    for await (const event of resultStream) {
      ...
      if (event.type === ACoderEventType.Finished && this.contextManager) {
      ...
      this.updateTelemetryTokenCount();
      if (event.type === ACoderEventType.Error) {
        isError = true;
      }
    }
  ...
  async *sendMessageStream(
    ...
    if (hooksEnabled && messageBus) {
      ...
      if (hookResult) {
        if (
          'type' in hookResult &&
          hookResult.type === ACoderEventType.AgentExecutionStopped
        ) {
          ...
          'type' in hookResult &&
          hookResult.type === ACoderEventType.AgentExecutionBlocked
        ) {
          yield hookResult;
          return new Turn(this.getChat(), prompt_id);
