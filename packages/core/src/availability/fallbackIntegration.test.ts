...
describe('Fallback Integration', () => {
  ...
  beforeEach(() => {
    ...
    config = {
      getModel: () => PREVIEW_A_CODER_MODEL_AUTO,
      getActiveModel: () => PREVIEW_A_CODER_MODEL_AUTO,
  ...
  it('should select fallback model when primary model is terminal and config is in AUTO mode', () => {
    ...
    // The policy chain for PREVIEW_A_CODER_MODEL_AUTO is [PREVIEW_A_CODER_MODEL, PREVIEW_A_CODER_FLASH_MODEL]
    availabilityService.markTerminal(PREVIEW_A_CODER_MODEL, 'quota');

    // 2. Request "Pro" explicitly (as Agent would)
    const requestedModel = PREVIEW_A_CODER_MODEL;
    ...
    expect(result.model).toBe(PREVIEW_A_CODER_FLASH_MODEL);
  ...
  it('should fallback for Gemini 3 models even if config is NOT in AUTO mode', () => {
    ...
    vi.spyOn(config, 'getModel').mockReturnValue(PREVIEW_A_CODER_MODEL);
  ...
  it('should fallback to Flash after failures and restore Pro on next turn', () => {
    ...
    expect(result1.model).toBe(PREVIEW_A_CODER_MODEL);
    expect(result1.maxAttempts).toBe(3);

    // 2. Simulate failure and transition to sticky_retry with consumed=true
    availabilityService.markRetryOncePerTurn(PREVIEW_A_CODER_MODEL, 3);
    availabilityService.consumeStickyAttempt(PREVIEW_A_CODER_MODEL);
    ...
    expect(result2.model).toBe(PREVIEW_A_CODER_FLASH_MODEL);
    ...
    expect(result3.model).toBe(PREVIEW_A_CODER_MODEL);
