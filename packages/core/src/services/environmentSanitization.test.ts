...
describe('sanitizeEnvironment', () => {
  ...
  it('should allow variables prefixed with A_CODER_CLI_', () => {
    const env = {
      A_CODER_CLI_FOO: 'bar',
      A_CODER_CLI_BAZ: 'qux',
    };
  ...
  describe('value-first security: secret values must be caught even for allowed variable names', () => {
    ...
    it('should NOT redact A_CODER_CLI_ variables even if their value looks like a secret (fully trusted)', () => {
      const env = {
        A_CODER_CLI_INTERNAL: 'ghp_xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
      };
      const sanitized = sanitizeEnvironment(env, EMPTY_OPTIONS);
      expect(sanitized).toEqual(env);
    });
