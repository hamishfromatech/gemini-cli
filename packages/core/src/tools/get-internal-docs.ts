...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find A-Coder CLI documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available A-Coder CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to A-Coder CLI's internal documentation.
