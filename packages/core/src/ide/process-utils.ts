 ...
 * process).
 *
 * This function can be overridden by setting the `A_CODER_CLI_IDE_PID`
 * environment variable. This is useful for launching A-Coder CLI in a
 * standalone terminal while still connecting to an IDE instance.
 *
 * If `A_CODER_CLI_IDE_PID` is set, the function uses that PID and fetches
 * the command for it.
 *
...
export async function getIdeProcessInfo(): Promise<{
  ...
  if (process.env['A_CODER_CLI_IDE_PID']) {
    const idePid = parseInt(process.env['A_CODER_CLI_IDE_PID'], 10);
