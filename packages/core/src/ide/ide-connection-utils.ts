...
export function validateWorkspacePath(
  ...
  if (!isWithinWorkspace) {
    return {
      isValid: false,
      error: `Directory mismatch. A-Coder CLI is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
        ', ',
      )}`,
    };
  }
...
export function getPortFromEnv(): string | undefined {
  const port = process.env['A_CODER_CLI_IDE_SERVER_PORT'];
  if (!port) {
    return undefined;
  }
  return port;
...
export function getStdioConfigFromEnv(): StdioConfig | undefined {
  const command = process.env['A_CODER_CLI_IDE_SERVER_STDIO_COMMAND'];
  if (!command) {
    return undefined;
  }

  const argsStr = process.env['A_CODER_CLI_IDE_SERVER_STDIO_ARGS'];
  let args: string[] = [];
  if (argsStr) {
    try {
      ...
      if (Array.isArray(parsedArgs)) {
        ...
        logger.error(
          'A_CODER_CLI_IDE_SERVER_STDIO_ARGS must be a JSON array string.',
        );
      }
    } catch (e) {
      logger.error('Failed to parse A_CODER_CLI_IDE_SERVER_STDIO_ARGS:', e);
    }
  }
...
const IDE_SERVER_FILE_REGEX = /^a-coder-ide-server-(\d+)-\d+\.json$/;
...
> {
  ...
  try {
    ...
    // /^a-coder-ide-server-${pid}-\d+\.json$/. If multiple IDE
