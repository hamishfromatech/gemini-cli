...
describe('Config Agents Reload Integration', () => {
  ...
  beforeEach(async () => {
    ...
    // Create the .a-coder/agents directory structure
    await fs.mkdir(path.join(tmpDir, '.a-coder', 'agents'), { recursive: true });
  });
  ...
  it('should unregister agents from the agent registry when they are disabled after being enabled', async () => {
    ...
    const agentPath = path.join(tmpDir, '.a-coder', 'agents', `${agentName}.md`);
