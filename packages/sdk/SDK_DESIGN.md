...
```ts
import { GeminiCliAgent } from '@the-a-tech-corporation/sdk';
...
- Static string instructions show up where A_CODER.md content normally would in
  model call
...
```ts
import { GeminiCliAgent, tool, z } from "@the-a-tech-corporation/sdk";
...
```ts
import { GeminiCliAgent, hook, z } from '@the-a-tech-corporation/sdk';
...
const myHook = hook(
  ...
  (hook, ctx) => {
    ...
    // ctx.fs gives us a filesystem interface that obeys A-Coder CLI permissions/sandbox
...
```ts
import { hook } from "@the-a-tech-corporation/sdk";
...
```ts
import { GeminiCliAgent, subagent } from "@the-a-tech-corporation/a-coder-cli";
...
Potentially the most important feature of the A-Coder CLI SDK is support for
extensions, which modularly encapsulate all of the primitives listed above:

```ts
import { GeminiCliAgent, extension } from "@the-a-tech-corporation/sdk";
...
```ts
import { GeminiCliAgent } from "@the-a-tech-corporation/sdk";
import { GeminiCliAcpServer } from "@the-a-tech-corporation/sdk/acp";
...
export interface AgentShellProcess {
  ...
  // investigate how A-Coder CLI already does this
}
```
