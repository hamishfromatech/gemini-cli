...
import { TestRig, A_CODER_DIR } from './test-helper.js';
...
describe('User Policy Regression Repro', () => {
  ...
  it('should respect policies in ~/.a-coder/policies/allowed-tools.toml', async () => {
    rig.setup('user-policy-test', {
      fakeResponsesPath: join(import.meta.dirname, 'user-policy.responses'),
    });

    // Create ~/.a-coder/policies/allowed-tools.toml
    const userPoliciesDir = join(rig.homeDir!, A_CODER_DIR, 'policies');
