...
export class ComponentRig {
  ...
  async initialize() {
    ...
    // This must happen BEFORE stubbing A_CODER_CLI_HOME because OAuth credential
    // lookup resolves through homedir() → A_CODER_CLI_HOME.
    await this.config.refreshAuth(AuthType.USE_GEMINI);

    // Isolate storage paths (session files, skills, extraction state) by
    // pointing A_CODER_CLI_HOME at a per-test temp directory.  Storage resolves
    ...
    vi.stubEnv('A_CODER_CLI_HOME', this.homeDir);
  }
