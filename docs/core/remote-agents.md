...
A-Coder CLI supports connecting to remote subagents using the Agent-to-Agent
(A2A) protocol. This allows A-Coder CLI to interact with other agents, expanding
its capabilities by delegating tasks to remote services.

A-Coder CLI can connect to any compliant A2A agent. You can find samples of A2A
...
A-Coder CLI routes traffic to remote agents through an HTTP/HTTPS proxy if one is
...
Many remote agents require authentication. A-Coder CLI supports several
...
A-Coder CLI supports the following authentication types:
...
use, A-Coder CLI opens your browser for sign-in and persists the resulting tokens
...
When A-Coder CLI loads a remote agent, it validates your auth configuration
...
When connecting to a remote agent, A-Coder CLI first fetches the agent card
...
Users can manage subagents using the following commands within A-Coder CLI:
...
<!-- prettier-ignore -->
> [!TIP]
> You can use the `@cli_help` agent within A-Coder CLI for assistance
