# A-Coder CLI extension best practices
...
Developing extensions for A-Coder CLI is a lightweight, iterative process. Use
...
your `a-coder-cli-extension.json` file:
...
a-coder-cli extensions install github.com/user/repo

# Install the development version
a-coder-cli extensions install github.com/user/repo --ref dev
```

### Clean artifacts

When using GitHub Releases, ensure your archives only contain necessary files
(such as `dist/`, `a-coder-cli-extension.json`, and `package.json`). Exclude
...
- **Check the manifest:** Ensure `a-coder-cli-extension.json` is in the root
...
- **Restart the CLI:** Extensions are loaded at the start of a session. Restart
  A-Coder CLI after making changes to the manifest or linking a new extension.
...
- **Test the command:** Run the server's `command` and `args` directly in your
  terminal to ensure it starts correctly outside of A-Coder CLI.
