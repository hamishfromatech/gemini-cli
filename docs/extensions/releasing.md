...
Release A-Coder CLI extensions to your users through a Git repository or GitHub
...
The [A-Coder CLI extension gallery](https://a-coder-cli.com/extensions/browse/)
...
1.  **Use a public repository:** Ensure your extension is hosted in a public
    GitHub repository.
2.  **Add the GitHub topic:** Add the `a-coder-cli-cli-extension` topic to your
    repository's **About** section. Our crawler uses this topic to find new
    extensions.
3.  **Place the manifest at the root:** Ensure your `a-coder-cli-extension.json` file
...
extension using `a-coder-cli extensions install <your-repo-uri>`.

Users can optionally depend on a specific branch, tag, or commit using the
`--ref` argument. For example:

```bash
a-coder-cli extensions install <your-repo-uri> --ref=stable
```
...
A-Coder CLI checks for updates by looking for the **Latest** release on GitHub.
...
To let A-Coder CLI find the correct asset for a user's platform, use the
...
Archives must be fully contained extensions. The `a-coder-cli-extension.json` file
...
`migratedTo` property in `a-coder-cli-extension.json` to seamlessly transition your
users.
...
2.  **Update the old repository:** In your original repository, update the
    `a-coder-cli-extension.json` file to include the `migratedTo` property pointing
...
When users check for updates, A-Coder CLI detects the `migratedTo` field,
...
A-Coder CLI automatically checks for extension updates based on the installation
method. Understanding these mechanisms helps you ensure your users always have
the latest version.

### Sync manifest and tags

For GitHub releases, always ensure the `version` in `a-coder-cli-extension.json`
...
metadata file at `~/.a-coder-cli/extensions/<name>/.a-coder-cli-extension-install.json`.
