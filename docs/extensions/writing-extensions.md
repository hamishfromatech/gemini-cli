# Build A-Coder CLI extensions

A-Coder CLI extensions let you expand the capabilities of A-Coder CLI by adding
...
```bash
a-coder-cli extensions new my-first-extension mcp-server
```
...
The manifest file tells A-Coder CLI how to load and use your extension.
...
1.  Open `a-coder-cli-extension.json`.
...
When a user installs this extension, A-Coder CLI will prompt them to enter the
...
Link your extension to your A-Coder CLI installation for local development.
...
2.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from A-Coder CLI extensions
    directory to your development directory. Changes you make are reflected
    immediately.

    ```bash
    a-coder-cli extensions link .
    ```

Restart your A-Coder CLI session to use the new `fetch_posts` tool. Test it by
asking: "fetch posts".
...
2.  Update your `a-coder-cli-extension.json` to load this file:
