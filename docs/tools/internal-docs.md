...
The `get_internal_docs` tool lets A-Coder CLI access its own technical
documentation to provide more accurate answers about its capabilities and usage.

## Description

This tool is used when A-Coder CLI needs to verify specific details about A-Coder
...
The `get_internal_docs` tool is used exclusively by A-Coder CLI. You cannot
invoke this tool manually.

When A-Coder CLI uses this tool, it retrieves the content of the requested
...
A-Coder CLI uses this tool to ensure technical accuracy:

- **Capability discovery:** If A-Coder CLI is unsure how a feature works, it can
  lookup the corresponding documentation.
- **Reference lookup:** A-Coder CLI can verify slash command sub-commands or
  specific setting names.
- **Self-correction:** A-Coder CLI can use the documentation to correct its
  understanding of A-Coder CLI's system logic.
