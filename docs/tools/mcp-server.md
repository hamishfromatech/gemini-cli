...
An MCP server is an application that exposes tools and resources to the A-Coder
CLI through the Model Context Protocol, allowing it to interact with external
systems and data sources. MCP servers act as a bridge between the A-Coder model
and your local environment or other services like APIs.

An MCP server enables A-Coder CLI to:
...
With an MCP server, you can extend A-Coder CLI's capabilities to perform actions
beyond its built-in features, such as interacting with databases, APIs, custom
scripts, or specialized workflows.

## Core integration architecture

A-Coder CLI integrates with MCP servers through a sophisticated discovery and
...
4. **Sanitizes and validates** tool schemas for compatibility with the A-Coder
   API
...
A-Coder CLI supports three MCP transport types:
...
prompts. A-Coder CLI discovers these automatically and gives you the possibility
...
A-Coder CLI uses the `mcpServers` configuration in your `settings.json` file to
...
A-Coder CLI automatically expands environment variables in the `env` block of
...
To protect your credentials, A-Coder CLI performs environment sanitization when
...
- Core project keys: `A_CODER_API_KEY`, `GOOGLE_API_KEY`, etc.
...
A-Coder CLI supports OAuth 2.0 authentication for remote MCP servers using SSE or
...
- **Stored securely** in `~/.a-coder-cli/mcp-oauth-tokens.json`
...
When A-Coder CLI starts, it performs MCP server discovery through the following
detailed process:
...
4. **Name sanitization:** Tool names are cleaned to meet A-Coder API
   requirements:
...
Tool parameter schemas undergo sanitization for A-Coder API compatibility:
...
When the A-Coder model decides to use an MCP tool, the following execution flow
occurs:
...
Once discovered, MCP tools are available to the A-Coder model like built-in
...
`settings.json`. A-Coder CLI merges your local configuration with the extension's
defaults:
...
- **Access tokens:** Be security-aware when configuring environment variables
  ...
  for details on how A-Coder CLI protects your credentials.
...
- **Property stripping:** The system automatically removes certain schema
  properties (`$schema`, `additionalProperties`) for A-Coder API compatibility
...
A-Coder CLI's capabilities while maintaining security, reliability, and ease of
use.
...
A-Coder CLI will correctly process this array, separating text from binary data
...
When A-Coder CLI receives this response, it will:
...
context to the A-Coder model.

## MCP prompts as slash commands

In addition to tools, MCP servers can expose predefined prompts that can be
executed as slash commands within A-Coder CLI. This lets you create shortcuts for
...
```bash
/poem-writer --title="A-Coder CLI" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "A-Coder CLI" reverent
```

When you run this command, A-Coder CLI executes the `prompts/get` method on the
...
## Managing MCP servers with `a-coder-cli mcp`

While you can always configure MCP servers by manually editing your
`settings.json` file, A-Coder CLI provides a convenient set of commands to manage
...
### Adding a server (`a-coder-cli mcp add`)
...
`~/.a-coder-cli/settings.json` or the project config `.a-coder-cli/settings.json` file.

**Command:**

```bash
a-coder-cli mcp add [options] <name> <commandOrUrl> [args...]
```
...
```bash
# Basic syntax
a-coder-cli mcp add [options] <name> <command> [args...]

# Example: Adding a local server
a-coder-cli mcp add -e API_KEY=123 -e DEBUG=true my-stdio-server /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
a-coder-cli mcp add python-server python server.py -- --server-arg my-value
```
...
```bash
# Basic syntax
a-coder-cli mcp add --transport http <name> <url>

# Example: Adding an HTTP server
a-coder-cli mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
a-coder-cli mcp add --transport http --header "Authorization: Bearer abc123" secure-http https://api.example.com/mcp/
```
...
```bash
# Basic syntax
a-coder-cli mcp add --transport sse <name> <url>

# Example: Adding an SSE server
a-coder-cli mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
a-coder-cli mcp add --transport sse --header "Authorization: Bearer abc123" secure-sse https://api.example.com/sse/
```

### Listing servers (`a-coder-cli mcp list`)
...
> "Disconnected". Use `a-coder-cli trust` to trust the current folder.
...
You can also use `a-coder-cli mcp list` from your shell to see connection errors for
all configured servers.

### Removing a server (`a-coder-cli mcp remove`)
...
```bash
a-coder-cli mcp remove my-server
```
...
### Enabling/disabling a server (`a-coder-cli mcp enable`, `a-coder-cli mcp disable`)
...
```bash
a-coder-cli mcp enable <name> [--session]
a-coder-cli mcp disable <name> [--session]
```
...
`~/.a-coder-cli/mcp-server-enablement.json`.
