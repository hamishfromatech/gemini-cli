...
A-Coder CLI can integrate with your IDE to provide a more seamless and
...
There are two primary ways to integrate A-Coder CLI with an IDE:

1.  **VS Code companion extension**: Install the "A-Coder CLI Companion"
...
2.  **Agent Client Protocol (ACP)**: An open protocol for interoperability
    ...
    discovery and installation of compatible agents like A-Coder CLI.

## VS Code companion extension

The **A-Coder CLI Companion extension** grants A-Coder CLI direct access to your
...
- **Native diffing:** When A-Coder suggests code modifications, you can view the
...
- **VS Code commands:** You can access A-Coder CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `A-Coder CLI: Run`: Starts a new A-Coder CLI session in the integrated
    terminal.
  - `A-Coder CLI: Accept Diff`: Accepts the changes in the active diff editor.
  - `A-Coder CLI: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `A-Coder CLI: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run A-Coder CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside A-Coder CLI:
...
- **For Visual Studio Code:** Install from the
  [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=google.a-coder-cli-cli-vscode-ide-companion).
- **For VS Code forks:** To support forks of VS Code, the extension is also
  published on the
  [Open VSX Registry](https://open-vsx.org/extension/google/a-coder-cli-cli-vscode-ide-companion).
  Follow your editor's instructions for installing extensions from this
  registry.

<!-- prettier-ignore -->
> [!NOTE]
> The "A-Coder CLI Companion" extension may appear towards the bottom of
...
When enabled, A-Coder CLI will automatically attempt to connect to the IDE
companion extension.
...
When you ask A-Coder to modify a file, it can open a diff view directly in your
editor.
...
- Open the Command Palette and run **A-Coder CLI: Accept Diff**.
...
- Open the Command Palette and run **A-Coder CLI: Close Diff Editor**.
...
A-Coder CLI is officially available in the **ACP Agent Registry**. This allows
you to install and update A-Coder CLI directly within supporting IDEs and
...
A-Coder CLI is an ACP-compatible agent available in the ACP Agent Registry.
...
Any other IDE that supports the ACP Agent Registry can install A-Coder CLI
directly through their in-built registry features.

## Using with sandboxing

If you are using A-Coder CLI within a sandbox, be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run A-Coder CLI inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Cause:** A-Coder CLI could not find the necessary environment variables
    (`A_CODER_CLI_IDE_WORKSPACE_PATH` or `A_CODER_CLI_IDE_SERVER_PORT`) to connect
    to the IDE. This usually means the IDE companion extension is not running or
    did not initialize correctly.
  - **Solution:**
    1.  Make sure you have installed the **A-Coder CLI Companion** extension in
...
If automatic IDE detection fails, or if you are running A-Coder CLI in a
standalone terminal and want to manually associate it with a specific IDE
instance, you can set the `A_CODER_CLI_IDE_PID` environment variable to the
process ID (PID) of your IDE.

**macOS/Linux**

```bash
export A_CODER_CLI_IDE_PID=12345
```

**Windows (PowerShell)**

```powershell
$env:A_CODER_CLI_IDE_PID=12345
```

When this variable is set, A-Coder CLI will skip automatic detection and attempt
to connect using the provided PID.

#### Configuration errors

- **Message:**
  `🔴 Disconnected: Directory mismatch. A-Coder CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run A-Coder CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running A-Coder CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run A-Coder CLI from the integrated terminal of a supported
    IDE, like Antigravity or VS Code.

- **Message:**
  `No installer is available for IDE. Please install A-Coder CLI Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "A-Coder CLI
    Companion", and
