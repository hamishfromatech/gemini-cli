# A-Coder CLI companion plugin: Interface specification

> Last Updated: September 15, 2025

This document defines the contract for building a companion plugin to enable
A-Coder CLI's IDE mode. For VS Code, these features (native diffing, context
awareness) are provided by the official extension
([marketplace](https://marketplace.visualstudio.com/items?itemName=Google.a-coder-cli-cli-vscode-ide-companion)).
...
A-Coder CLI and the IDE plugin communicate through a local communication channel.
...
For A-Coder CLI to connect, it needs to discover which IDE instance it's running
...
- **File location:** The file must be created in a specific directory:
  `os.tmpdir()/a-coder-cli/ide/`. Your plugin must create this directory if it
  doesn't exist.
- **File naming convention:** The filename is critical and **MUST** follow the
  pattern: `a-coder-cli-ide-server-${PID}-${PORT}.json`
...
- **Tie-breaking with environment variables (recommended):** For the most
  ...
  set the `A_CODER_CLI_IDE_SERVER_PORT` environment variable in the integrated
  ...
  `A_CODER_CLI_IDE_SERVER_PORT` variable to identify and connect to the correct
  window's server.
