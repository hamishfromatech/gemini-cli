...
A-Coder CLI also records the command and its output in the current session
...
You can automate tasks using a combination of A-Coder CLI and shell commands.
...
1.  A-Coder calls `run_shell_command('npm test')`.
2.  You see a confirmation prompt: `Allow command 'npm test'? [y/N]`.
3.  You press `y`.
4.  The tests run. If they fail, A-Coder reads the error output.
5.  A-Coder uses `read_file` to inspect the failing test.
6.  A-Coder uses `replace` to fix the bug.
7.  A-Coder runs `npm test` again to verify the fix.

This loop lets A-Coder work autonomously.

## How to manage background processes

You can ask A-Coder to start long-running tasks, like development servers or file
watchers.

**Prompt:** `Start the React dev server in the background.`

A-Coder will run the command (for example, `npm run dev`) and detach it.
...
A-Coder CLI attempts to handle interactive commands (like `git add -p` or
...
Giving an AI access to your shell is powerful but risky. A-Coder CLI includes
