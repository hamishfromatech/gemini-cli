...
Connect A-Coder CLI to your external databases and services. In this guide,
you'll learn how to extend A-Coder CLI's capabilities by installing the GitHub
MCP server and using it to manage your repositories.

## Prerequisites

- A-Coder CLI installed.
...
## How to configure A-Coder CLI

You tell A-Coder about new servers by editing your `settings.json`.

1.  Open `~/.a-coder-cli/settings.json` (or the project-specific
    `.a-coder-cli/settings.json`).
2.  Add the `mcpServers` block. This tells A-Coder: "Run this docker container
    and talk to it."
...
Restart A-Coder CLI. It will automatically try to start the defined servers.
...
**Prompt:** `List the open PRs in the google/a-coder-cli-cli repository.`
