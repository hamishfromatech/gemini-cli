# System Prompt Override (A_CODER_SYSTEM_MD)

The core system instructions that guide A-Coder CLI can be completely replaced
with your own Markdown file. This feature is controlled via the
`A_CODER_SYSTEM_MD` environment variable.

## Overview

The `A_CODER_SYSTEM_MD` variable instructs the CLI to use an external Markdown
...
via a `.a-coder-cli/.env` file. See
[Persisting Environment Variables](../get-started/authentication.mdx#persisting-environment-variables).

- Use the project default path (`.a-coder-cli/system.md`):
  - `A_CODER_SYSTEM_MD=true` or `A_CODER_SYSTEM_MD=1`
  - The CLI reads `./.a-coder-cli/system.md` (relative to your current project
    directory).

- Use a custom file path:
  - `A_CODER_SYSTEM_MD=/absolute/path/to/my-system.md`
...
- Disable the override (use built‑in prompt):
  - `A_CODER_SYSTEM_MD=false` or `A_CODER_SYSTEM_MD=0` or unset the variable.
...
- One‑off session using a project file:
  - `A_CODER_SYSTEM_MD=1 a-coder-cli`
- Persist for a project using `.a-coder-cli/.env`:
  - Create `.a-coder-cli/system.md`, then add to `.a-coder-cli/.env`:
    - `A_CODER_SYSTEM_MD=1`
- Use a custom file under your home directory:
  - `A_CODER_SYSTEM_MD=~/prompts/system.md a-coder-cli`

## UI indicator

When `A_CODER_SYSTEM_MD` is active, the CLI shows a `|⌐■_■|` indicator in the UI
...
- Write the built‑in prompt to the project default path:
  - `A_CODER_WRITE_SYSTEM_MD=1 a-coder-cli`
- Or write to a custom path:
  - `A_CODER_WRITE_SYSTEM_MD=~/prompts/DEFAULT_SYSTEM.md a-coder-cli`
