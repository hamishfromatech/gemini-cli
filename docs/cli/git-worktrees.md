...
A-Coder session its own copy of the codebase. Git worktrees create separate
...
>
> - [Open an issue](https://github.com/google-a-coder-cli/a-coder-cli-cli/issues/new?template=bug_report.yml) on GitHub.
> - Use the **/bug** command within A-Coder CLI to file an issue.
...
Use the `--worktree` (`-w`) flag to create an isolated worktree and start A-Coder
CLI in it.

- **Start with a specific name:** The value you pass becomes both the directory
  name (within `.a-coder-cli/worktrees/`) and the branch name.

  ```bash
  a-coder-cli --worktree feature-search
  ```

- **Start with a random name:** If you omit the name, A-Coder generates a random
  one automatically (for example, `worktree-a1b2c3d4`).

  ```bash
  a-coder-cli --worktree
  ```
...
When you exit a worktree session (using `/quit` or `Ctrl+C`), A-Coder leaves the
...
A-Coder prioritizes a fast and safe exit: it **does not automatically delete**
your worktree or branch. You are responsible for cleaning up your worktrees
manually once you are finished with them.

When you exit, A-Coder displays instructions on how to resume your work or how to
...
A-Coder CLI with the `--resume` flag and the session ID:

```bash
cd .a-coder-cli/worktrees/feature-search
a-coder-cli --resume <session_id>
```
...
- **Clean up a preserved Git worktree:**
  ```bash
  git worktree remove .a-coder-cli/worktrees/feature-search --force
...
- **Create a Git worktree manually:**
  ...
  cd ../project-feature-search && a-coder-cli
  ```
