...
A-Coder CLI includes a model routing feature that automatically switches to a
...
2.  **User consent:** Depending on the failure and the model's policy, the CLI
    ...
    use a silent fallback chain for `a-coder-cli-2.5-flash-lite` and will fall back
    to `a-coder-cli-2.5-flash` and `a-coder-cli-2.5-pro` without prompting or changing the
    configured model.
...
A-Coder CLI supports using a local model for routing decisions. When configured,
A-Coder CLI will use a locally-running **Gemma** model to make routing decisions
...
The easiest way to set this up is using the automated `a-coder-cli gemma setup`
command.

For more details on how to configure local model routing, see
[`a-coder-cli gemma` — Local Model Routing Setup](../core/gemma-setup.md).

### Model selection precedence

The model used by A-Coder CLI is determined by the following order of precedence:

1.  **`--model` command-line flag:** A model specified with the `--model` flag
    when launching the CLI will always be used.
2.  **`A_CODER_MODEL` environment variable:** If the `--model` flag is not used,
    the CLI will use the model specified in the `A_CODER_MODEL` environment
    variable.
...
4.  **Local model (experimental):** If the Gemma local model router is enabled
    ...
    (instead of A-Coder models) to route the request to an appropriate model.
