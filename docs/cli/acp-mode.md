...
ACP (Agent Client Protocol) mode is a special operational mode of A-Coder CLI
...
A-Coder CLI agent and a client.

To start A-Coder CLI in ACP mode, use the `--acp` flag:
...
ACP-compatible agents across various IDEs. A-Coder CLI is an ACP-compatible agent
...
- **Client's role:** The client is responsible for sending requests (for
  example, prompts) and handling responses and notifications from A-Coder CLI.
- **A-Coder CLI's role:** In ACP mode, A-Coder CLI listens for incoming JSON-RPC
...
(like an IDE) expose its own functionality as "tools" that the A-Coder model can
use.
...
3.  A-Coder CLI connects to the MCP server, discovers the available tools, and
    makes them available to the AI model.
4.  When the model decides to use one of these tools, A-Coder CLI sends a tool
...
To enable general debugging logs, start A-Coder CLI with the `--debug` flag:

```bash
a-coder-cli --acp --debug
```
...
- `A_CODER_TELEMETRY_TARGET=local`
- `A_CODER_TELEMETRY_OUTFILE=/path/to/your/log.json`
