...
Agent Skills provide A-Coder CLI with specialized expertise on demand. This guide
...
1.  **Built-in Skills**: Included with A-Coder CLI and always available.
...
The `a-coder-cli skills` command provides management utilities directly from your
system shell.
...
```bash
a-coder-cli skills install https://github.com/user/my-awesome-skill
```
...
```bash
a-coder-cli skills link ./path/to/my-skill
```
...
```bash
a-coder-cli skills uninstall <name>
```
