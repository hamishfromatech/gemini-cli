...
- **Location:** Sessions are stored in `~/.a-coder-cli/tmp/<project_hash>/chats/`,
...
When starting A-Coder CLI, use the `--resume` (or `-r`) flag to load existing
sessions.
...
- **Resume by index:** List available sessions first (see
  ...
  ```bash
  a-coder-cli --resume 1
  ```

- **Resume by ID:** You can also provide the full session UUID:
  ```bash
  a-coder-cli --resume a1b2c3d4-e5f6-7890-abcd-ef1234567890
  ```
...
[Git worktrees](./git-worktrees.md) to give each A-Coder session its own copy of
...
You can configure how A-Coder CLI manages your session history in your
`settings.json` file. These settings let you control retention policies and
session lengths.

### Session retention

By default, A-Coder CLI automatically cleans up old session data to prevent your
history from growing indefinitely. When a session is deleted, A-Coder CLI also
