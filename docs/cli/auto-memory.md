...
Auto Memory is an experimental feature that mines your past A-Coder CLI sessions
...
Every session you run with A-Coder CLI is recorded locally as a transcript. Auto
...
1.  Open your global settings file at `~/.a-coder-cli/settings.json`. If you only
    want Auto Memory in one project, edit `.a-coder-cli/settings.json` in that
    project instead.
...
3.  Restart A-Coder CLI. The flag requires a restart because the extraction
...
1.  **Eligibility scan.** The service indexes recent sessions from
    `~/.a-coder-cli/tmp/<project>/chats/`. Sessions are eligible only if they have
...
6.  **Notification.** When a run produces new candidates, A-Coder CLI surfaces an
...
- **Promote** a skill to your user (`~/.a-coder-cli/skills/`) or workspace
  (`.a-coder-cli/skills/`) directory.
...
- The extraction agent runs on a preview A-Coder Flash model. Extraction quality
...
- Project or workspace shared instructions in project `A_CODER.md` files are not
...
- Explore the [memory management tutorial](./tutorials/memory-management.md) for
  the complementary explicit-memory and `A_CODER.md` workflows.
