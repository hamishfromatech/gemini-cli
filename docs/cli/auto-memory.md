...
1.  Open your global settings file at `~/.a-coder-cli/settings.json`. If you
    only want Auto Memory in one project, edit `.a-coder-cli/settings.json` in
...
1.  **Eligibility scan.** The service indexes recent sessions from
    `~/.a-coder-cli/tmp/<project>/chats/`. Sessions are eligible only if they
...
6.  **Notification.** When a run produces new candidates, A-Coder CLI surfaces
...
- Project or workspace shared instructions in project `A-Coder.md` files are not
...
- Explore the [memory management tutorial](./tutorials/memory-management.md) for
  the complementary explicit-memory and `A-Coder.md` workflows.
