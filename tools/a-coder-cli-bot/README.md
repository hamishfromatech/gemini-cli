# A-Coder CLI Bot (Cognitive Repository)

This directory contains the foundational architecture for the `a-coder-cli-bot`,
...
- **Frequency**: 30-minute cron (`.github/workflows/a-coder-cli-bot-pulse.yml`).
- **Implementation**: Pure TypeScript/JavaScript scripts.
- **Classification**: Optionally utilizes A-Coder CLI for high-confidence
...
- **Purpose**: Strategic investigation, policy refinement, and proactive
  self-optimization.
- **Frequency**: 24-hour cron (`.github/workflows/a-coder-cli-bot-brain.yml`).
- **Implementation**: Agentic A-Coder CLI phases.
- **Phases**:
...
```bash
npx tsx tools/a-coder-cli-bot/metrics/index.ts
```

This will execute all scripts within `metrics/scripts/` and output the results
to `tools/a-coder-cli-bot/history/metrics-before.csv`.
