...
- Time-series repository metrics are stored in
  `tools/a-coder-cli-bot/history/metrics-timeseries.csv`.
- Recent point-in-time metrics are in
  `tools/a-coder-cli-bot/history/metrics-before-prev.csv` and the current run's
  metrics.
...
You are explicitly authorized to use the A-Coder CLI (`bundle/gemini.js`) within
...
- **Strict Role Separation**: Use A-Coder CLI ONLY for **classification** (data
...
- **Default Policy Enforcement**: When generating scripts that invoke Gemini
  CLI, they MUST NOT use the specialized `tools/a-coder-cli-bot/ci-policy.toml`.
...
- Load and analyze `tools/a-coder-cli-bot/history/metrics-timeseries.csv`.
...
- **Review Existing Policies**: Examine the existing automation in
  `.github/workflows/` and scripts in `tools/a-coder-cli-bot/reflexes/scripts/`.
...
- Summarize your findings for the Orchestrator. When modifying scripts in
  `tools/a-coder-cli-bot/metrics/scripts/`, you MUST NEVER change the output
